...
export default async function ComplexExternalNetworkPage({
  ...
  try {
    const network = await getComplexExtNetwork(complexId, limit, offset);
...
// 更稳的分页判断：
// 如果后端返回了 total_edges / total，就用总数判断；
// 如果没有可靠 total，就只要当前页装满了 limit，就允许继续点 Next。
...
const endItem = offset + network.edges.length;
  ...
  <p>
    Page {currentPage}: showing external interaction batch{" "}
    {startItem}-{endItem}
    {hasReliableTotal ? ` of ${totalEdges}` : ""}.
  </p>

  <p>
    This page only displays one batch of external PPI edges connected to
    complex {complexId}. Use Next Page to view the next batch.
  </p>
