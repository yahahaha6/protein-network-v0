...
前端不根据 raw key、evidenceLevel 或字段是否存在推断 relationKind。视觉样式只消费后端返回的 relationKind 和透明证据 flags。

### 9.3 详情展示与导出

主详情区按固定顺序展示 relation identity、biological interpretation、interaction evidence、feature annotations、expression context 和 provenance。co-membership-only 必须使用“共同复合物成员，暂无独立互作证据”文案，不能显示 Low evidence PPI。
...
里程碑 A 的实施计划按 A1 evidence/count、A2 relation/provenance contract、A3 frontend view-model/export 三个提交序列执行，避免在一个改动中同时重写后端和大型前端组件。
...
退出条件：四类网络点击 edge 的 synthetic/browser tests 全部通过，无核心 raw fallback，不再把 co-membership 显示为低证据 PPI，证据 unknown 不显示为零。
